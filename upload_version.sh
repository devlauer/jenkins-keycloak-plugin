...
docker run --rm -v ${TRAVIS_BUILD_DIR}/update-center.key:/juseppe/cert/uc.key -v ${TRAVIS_BUILD_DIR}/update-center.crt:/juseppe/cert/uc.crt -v ${TRAVIS_BUILD_DIR}/plugins/:/juseppe/plugins/ -v ${TRAVIS_BUILD_DIR}/:/juseppe/json/ -e JUSEPPE_UPDATE_CENTER_ID=elnarion-jenkins-site -e JUSEPPE_BASE_URI=https://dl.bintray.com/devlauer/update-site/ lanwen/juseppe generate
...
curl --request PUT --upload-file ./plugins/keycloak.hpi --user ${CI_DEPLOY_USERNAME}:${CI_DEPLOY_PASSWORD} "https://api.bintray.com/content/devlauer/update-site/jenkins-plugins/latestupdates//keycloak.hpi;publish=1;override=1"
